...
class UpdateDatabase(Task):
    ...
    def run(self, cfg: Optional[DictConfig] = None):
        ...
        driver.pool(self.n_jobs).write(
            node_payload, callback=lambda _: pbar1.update(), chunksize=self.chunksize, error_callback=error_callback
        )

        pbar2 = tqdm(desc="writing edges", total=len(edge_payload))
        driver.pool(self.n_jobs).write(
            edge_payload, callback=lambda _: pbar2.update(), chunksize=self.chunksize, error_callback=error_callback
        )
