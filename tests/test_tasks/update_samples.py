...
def test_update_cfg(config: DictConfig):
    """This tests whether we can update the configuration and still get
...
def test_update_task(config: DictConfig):
    task = UpdateSampleDatabase(
        n_jobs=12, strict=False, log_level="DEBUG", query=Query(n_samples=10)
    )
    task(config)
